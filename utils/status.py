...
class ProgressBar:
    ...
    def prog(self, i: int, max_iter: int, epoch: Union[int, str], loss: float, **kwargs) -> None:
        """
        ...
        if i:  # not (i + 1) % 10 or (i + 1) == max_iter:
            ...
            stringa = '\r[ {} ] | epoch {}: |{}| {} ep/h | loss: {} |'
            stringa += ' | '.join([f'{k}: {v:.8f}' if isinstance(v, float) else f'{k}: {v}' for k, v in kwargs.items()])
...
def progress_bar(i: int, max_iter: int, epoch: Union[int, str], loss: float, **kwargs) -> None:
    """
    Prints out the progress bar on the stderr file.
    :param i: the current iteration
    :param max_iter: the maximum number of iteration
    ...
    static_bar.prog(i, max_iter, epoch, loss, **kwargs)
