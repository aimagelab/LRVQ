...
def get_parser(parser):
    """
    """
    parser.add_argument('--obs_len', type=int, default=10, help='Length of the observed trajectory')
    parser.add_argument('--pred_len', type=int, default=20, help='Length of the predicted trajectory')
    # parser.add_argument('--set', type=str, default='train', help='Dataset split to use')
...
class NBADataset(Dataset):
    ...
    def plot_single_scene(self, scene_idx, actors=-1):
        ...
        for i in range(scene[:actors].shape[0]):
            ax.plot(scene[i, 0, :self.obs_len], scene[i, 1, :self.obs_len], linewidth=.7, color='green', marker='o')
            ax.plot(scene[i, 0, self.obs_len:], scene[i, 1, self.obs_len:], linewidth=.7, color='red', marker='o')
        plt.show()
...
def data_loader(args, path, split, skip=1, shuffle=True, transform=None):
    ...
    split_path = '{}/nba_{}.npy'.format(npy_files_path, split)

    dataset = NBADataset(data_root=split_path, obs_len=args.obs_len, pred_len=args.pred_len, training=split == 'train')
    sampler = None
...
class NbaDataset(BaseDataset):
    ...
    def get_data_loaders(self) -> Tuple[DataLoader, DataLoader, DataLoader, None]:
        ...
        path = os.path.join(base_path_dataset(), 'nba')
